...
func process(source string, targetObjs []string, output string, includePaths []string, packName string) (string, error) {
	...
	tmpl := `// Code generated by fastrlp. DO NOT EDIT.
	package {{.Package}}
...
func generateUnmarshal(v *value) string {
	tmpl := `// UnmarshalRLP implements the Unmarshaler interface
	func (:: *{{.Name}}) UnmarshalRLP(buf []byte) error {
...
func generateMarshal(v *value) string {
	tmpl := `// MarshalRLP implements the Marshaler interface
	func (:: *{{.Name}}) MarshalRLP() []byte {
	...
	if v.hasHash() {
		hash = `
		// ComputeHash computes the hash of the object
		func (:: *` + v.name + `) ComputeHash() *` + v.name + ` {
			v := ::.MarshalRLP()
			k := fastrlp.NewKeccak256()
			k.Write(v)
			k.Sum(::.Hash[:])
			return ::
		}
